...
@Slf4j
@Component
public class RecommendationRequestConsumer extends AbstractNotification {
    private final MessageBuilderUtils<RecommendationEventDto> messageBuilderUtils;
    private final ObjectMapper objectMapper;

    public RecommendationRequestConsumer(MessageBuilderUtils<RecommendationEventDto> messageBuilderUtils,
    ...
    @KafkaListener(topics = "${spring.kafka.consumer.topics.recommendation-request}",
            containerFactory = "objectContainerFactory",
            groupId = "${spring.kafka.consumer.group-id.recommendation-request}")
