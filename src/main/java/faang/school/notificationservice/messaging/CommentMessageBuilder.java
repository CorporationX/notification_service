...
public class CommentMessageBuilder implements MessageBuilder<CommentEvent> {
    ...
    public String buildMessage(CommentEvent event, Locale locale) {
        UserDto userDto = userServiceClient.getUser(event.commentAuthorId());
        return messageSource.getMessage("comment.notification",
                new Object[]{userDto.getUsername(), event.postId(), event.content()}, locale);
    }
}
