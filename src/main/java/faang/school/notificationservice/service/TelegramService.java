...
public class TelegramService implements NotificationService {
    ...
    public static final String CANCEL_REGISTRATION_SUCCESS = "cancel.registration.success";
    public static final String CANCEL_REGISTRATION_ERROR = "cancel.registration.error";
    ...
    public void registerUser(Long chatId, String phone) {
        ...
        try {
            ...
            responseTelegramText.append(messages.get(REGISTRATION_SUCCESS));
        } catch (FeignException fe) {
            String errorMessage = utils.format(USER_BY_PHONE_NOT_FOUND, phone);
            responseTelegramText.append(messages.get(REGISTRATION_ERROR));
    ...
    public void unregisterUser(Long chatId) {
        ...
        try {
            ...
            responseTelegramText.append(messages.get(CANCEL_REGISTRATION_SUCCESS));
        } catch (FeignException fe) {
            String errorMessage = utils.format(TELEGRAM_NOT_FOUND, chatId);
            responseTelegramText.append(messages.get(CANCEL_REGISTRATION_ERROR));
            handlerFeignException(fe, errorMessage);
        } finally {
            telegramBot.removeKeyboardAfterReceivingContact(chatId, responseTelegramText.toString());
        }
    ...
    private void initMessages() {
        messages.put(REGISTRATION_SUCCESS, getMessage(REGISTRATION_SUCCESS));
        messages.put(REGISTRATION_ERROR, getMessage(REGISTRATION_ERROR));
        messages.put(CANCEL_REGISTRATION_SUCCESS, getMessage(CANCEL_REGISTRATION_SUCCESS));
        messages.put(CANCEL_REGISTRATION_ERROR, getMessage(CANCEL_REGISTRATION_ERROR));
